...
// MARK: Read_voltage
/**
 * @brief Task to read the potentiometer voltage
 * 
 * This function allows you to switch the potentiometer reading off and on.
 */
static esp_err_t http_server_get_pot_voltage_json_handler(httpd_req_t *req)
{
	ESP_LOGI(TAG_2, "/pot_volt.json requested");

	char pot_voltJSON[100];
	int Pot_volt = 0;

	// If the potentiometer value is received
	if (xQueueReceive(POT_V, &Pot_volt, 100 / portTICK_PERIOD_MS) == pdTRUE)
	{
		sprintf(pot_voltJSON, "{\"pot_volt\":\"%d\"}", Pot_volt);

		httpd_resp_set_type(req, "application/json");

		httpd_resp_send(req, pot_voltJSON, strlen(pot_voltJSON));
	}
	else
	{
		sprintf(pot_voltJSON, "{\"error\":\"Failed to read sensor data\"}");
		httpd_resp_set_type(req, "application/json");
		httpd_resp_send(req, pot_voltJSON, strlen(pot_voltJSON));
	}
...
// MARK: RGB_Values
/**
 * @brief Get rgb values from the RGB LED
 * @param req HTTP request for which the uri needs to be handled
 * @return ESP_OK
 */
static esp_err_t http_server_rgb_value_json_handler(httpd_req_t *req)
{

	ESP_LOGI(TAG_2, "/rgb_values.json requested");


	configure_led_rgb(RED_GPIO, GREEN_GPIO, BLUE_GPIO, LEDC_DUTY_RES);
	size_t header_len;
	char* header_value;
	int red = 0;
	int blue = 0;
	...
	if (red_json == NULL || green_json == NULL || blue_json == NULL || 
		!cJSON_IsNumber(red_json) || !cJSON_IsNumber(green_json) || !cJSON_IsNumber(blue_json)) {
		ESP_LOGE(TAG_2, "Missing or invalid RGB values");
		cJSON_Delete(root);
		return ESP_FAIL;
	}


	// Convertir los valores JSON a enteros
	red = red_json->valueint;
	green = green_json->valueint;
	blue = blue_json->valueint;

	// Aplicar los valores al LED RGB
	set_rgb_color(red, green, blue, LEDC_DUTY_RES);
	ESP_LOGI(TAG_2, "Parsed RGB values: R=%d, G=%d, B=%d", red, green, blue);
	// Set the type of the response
	httpd_resp_set_type(req, "application/json");
	// Send the response
	httpd_resp_send(req, "Send RGB", HTTPD_RESP_USE_STRLEN);
...
{
	ESP_LOGI(TAG_2, "/readreg.json requested");
...
static esp_err_t http_server_register_change_handler(httpd_req_t *req)
{
   ...
   // cJSON* day_item;
    //cJSON_ArrayForEach(day_item, selectedDays_json) {
       // if (cJSON_IsString(day_item)) {
         //   const char* day_str = day_item->valuestring;
           // ESP_LOGI(TAG_2, "Selected Day: %s", day_str);
...
static httpd_handle_t http_server_configure(void)
{
	...
	if (httpd_start(&http_server_handle, &config) == ESP_OK)
	{
		...
		// register register change handler
		...
		httpd_uri_t pot_voltage_json = {
			.uri = "/pot_volt.json",
			.method = HTTP_GET,
			.handler = http_server_get_pot_voltage_json_handler,
			.user_ctx = NULL
		};
		httpd_register_uri_handler(http_server_handle, &pot_voltage_json);

		// register rgb_values.json handler
		httpd_uri_t rgb_value ={
				.uri = "/rgb_values.json",
				.method = HTTP_POST,
				.handler = http_server_rgb_value_json_handler,
				.user_ctx = NULL
		};
		httpd_register_uri_handler(http_server_handle, &rgb_value);
