...
{	
	...
	tzset();
	ESP_LOGI(TAG_3, "Initializing SNTP");
	...
	{
		ESP_LOGI(TAG_3, "Waiting for system time to be set... (%d/%d)", retry, retry_count);
	...
	{
		ESP_LOGI(TAG_3, "System time is set!");
		time_was_synchronized = true;
	}
	else
	{
		ESP_LOGE(TAG_3, "Unable to set system time. Check your SNTP configuration.");
	}
}
...
//MARK: UPDATE_REG
/**
 * @brief Update the register information
 * @param reg_to_update register to update
 */
...
void connect_to_wifi(void) {
	...
	if (xSemaphoreTake(mySemaphore, portMAX_DELAY)) { // helpus to not allow multiple calls
		...
		send_uart_response("\nSSDI:");
		send_uart_response(ssid);
		send_uart_response("\nPassword:");
		send_uart_response(password);
...
//MARK: TASK_HOUR
/**
 * @brief Task to compare the hour and
 */
void task_compare_hour_to_execute_action( void ) {
	...
	while (1){
		...
		ESP_LOGI(TAG_3, "COMPARING HOURS");


		if (time(&now) != -1 && localtime_r(&now, &timeinfo) != NULL)
		{
			// Imprimir la hora actual
			// ESP_LOGI(TAG_3, "Día de la semana: %d, Hora: %02d:%02d:%02d", timeinfo.tm_wday, timeinfo.tm_hour, timeinfo.tm_min, timeinfo.tm_sec);
			ESP_LOGI(TAG_3, "Día de la semana: %d, Hora: %d:%d:%d", timeinfo.tm_wday, timeinfo.tm_hour, timeinfo.tm_min, timeinfo.tm_sec);
			
		}
		else
		{
			ESP_LOGE(TAG_3, "Error al obtener la hora actual.");
		}

		for(int i = 0; i< NUM_REGISTERS_AV; i++){
			ESP_LOGI(TAG_3, "Revisando registro: %d", i);
